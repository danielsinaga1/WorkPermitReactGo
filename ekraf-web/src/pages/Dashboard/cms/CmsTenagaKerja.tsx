...
import { tenagaKerjaService } from '../../../services';
import type { TenagaKerjaData } from '../../../types';
...
const CmsTenagaKerja = () => {
  const [items, setItems] = useState<TenagaKerjaData[]>([]);
  ...
  const [editItem, setEditItem] = useState<TenagaKerjaData | null>(null);
  const [form, setForm] = useState({ title: '', content: '', date: '', thumbnail: '', pdf_url: '', is_published: true });
  const [submitting, setSubmitting] = useState(false);

  const breadcrumbItems = [{ label: 'CMS' }, { label: 'Tenaga Kerja' }];
  ...
  const fetchData = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await tenagaKerjaService.list({ page, per_page: 10, search: search || undefined });
      setItems(res.data);
  ...
  const openEdit = (item: TenagaKerjaData) => {
    setEditItem(item);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      if (editItem) { await tenagaKerjaService.update(editItem.id, form); }
      else { await tenagaKerjaService.create(form); }
      setShowModal(false);
      fetchData();
  ...
  const confirmDelete = (id: number) => {
    confirmDialog({ message: 'Apakah Anda yakin ingin menghapus data ini?', header: 'Konfirmasi Hapus', icon: 'pi pi-exclamation-triangle', acceptClassName: 'p-button-danger', acceptLabel: 'Hapus', rejectLabel: 'Batal',
      accept: async () => { try { await tenagaKerjaService.destroy(id); fetchData(); } catch { alert('Gagal menghapus'); } },
    });
  };

  const statusBodyTemplate = (row: TenagaKerjaData) => (<Tag value={row.is_published ? 'Published' : 'Draft'} severity={row.is_published ? 'success' : 'warning'} className="text-xs" />);
  const dateBodyTemplate = (row: TenagaKerjaData) => (<span className="text-sm">{row.date ? new Date(row.date).toLocaleDateString('id-ID') : '-'}</span>);
  const pdfBodyTemplate = (row: TenagaKerjaData) => row.pdf_url ? <a href={row.pdf_url} target="_blank" rel="noreferrer" className="text-primary"><i className="pi pi-file-pdf mr-1" />PDF</a> : <span className="text-500">-</span>;
  const actionBodyTemplate = (row: TenagaKerjaData) => (
  ...
  const dialogFooter = (
    <div className="flex justify-content-end gap-2 pt-2">
      ...
      <Button label={submitting ? 'Menyimpan...' : 'Simpan'} icon="pi pi-check" loading={submitting} onClick={() => document.getElementById('tk-form')?.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }))} />
    </div>
  );
  ...
  return (
    <div className="flex flex-column gap-4">
      ...
      <div className="flex align-items-center justify-content-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-800 m-0">Tenaga Kerja</h2>
      ...
      <Card className="shadow-1 border-round-xl">
        ...
        <DataTable value={items} loading={loading} size="small" stripedRows responsiveLayout="scroll" emptyMessage="Belum ada data" className="border-round-lg">
          ...
          <Column header="PDF" body={pdfBodyTemplate} style={{ minWidth: '80px' }} />
      ...
      <Dialog visible={showModal} onHide={() => setShowModal(false)} header={editItem ? 'Edit Data' : 'Tambah Data Baru'} footer={dialogFooter} style={{ width: '540px' }} modal className="border-round-xl" draggable={false}>
        <form id="tk-form" onSubmit={handleSubmit} className="flex flex-column gap-3 pt-2">
