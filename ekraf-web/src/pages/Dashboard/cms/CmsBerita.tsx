...
import { beritaService } from '../../../services';
...
const CmsBerita = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await beritaService.list({ page, per_page: 10, search: search || undefined });
      setItems(res.data);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editItem) {
        await beritaService.update(editItem.id, form);
      } else {
  ...
  const confirmDelete = (id: number) => {
    confirmDialog({
      ...
      accept: async () => {
        try {
          await beritaService.destroy(id);
          fetchData();
        } catch {
  ...
  const statusBodyTemplate = (row: Berita) => (
    <Tag
      value={row.is_published ? 'Published' : 'Draft'}
      severity={row.is_published ? 'success' : 'warning'}
      className="text-xs"
    />
  );

  const dateBodyTemplate = (row: Berita) => (
    <span className="text-sm">{row.date ? new Date(row.date).toLocaleDateString('id-ID') : '-'}</span>
  );

  const authorBodyTemplate = (row: Berita) => (
    <span className="text-sm">{row.author?.name || '-'}</span>
  );

  const actionBodyTemplate = (row: Berita) => (
  ...
  const dialogFooter = (
    <div className="flex justify-content-end gap-2 pt-2">
      ...
      <Button label={submitting ? 'Menyimpan...' : 'Simpan'} icon="pi pi-check" loading={submitting} onClick={() => document.getElementById('berita-form')?.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }))} />
    </div>
  );
  ...
  return (
    <div className="flex flex-column gap-4">
      ...
      <div className="flex align-items-center justify-content-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-800 m-0">Berita & Artikel</h2>
          <BreadCrumb model={breadcrumbItems} home={breadcrumbHome} className="surface-ground border-none p-0 mt-2" />
        </div>
        <Button label="Buat Berita" icon="pi pi-plus" className="border-round-xl" onClick={openCreate} />
      </div>
      ...
      <Card className="shadow-1 border-round-xl">
        <div className="flex align-items-center justify-content-between mb-4 flex-wrap gap-3">
          <span className="p-input-icon-left">
            ...
            <InputText
              ...
              className="border-round-xl w-20rem"
            />
          </span>
          <span className="text-sm text-500">Total: {total} berita</span>
        </div>
        ...
        >
          ...
          <Column header="Penulis" body={authorBodyTemplate} style={{ minWidth: '120px' }} />
      ...
      <Dialog
        ...
        header={editItem ? 'Edit Berita' : 'Buat Berita Baru'}
        footer={dialogFooter}
        style={{ width: '540px' }}
        modal
        ...
        draggable={false}
      >
        <form id="berita-form" onSubmit={handleSubmit} className="flex flex-column gap-3 pt-2">
          ...
          <div className="flex flex-column gap-2">
            <label className="font-medium text-sm text-800">Konten</label>
          ...
          <div className="grid">
            <div className="col-6 flex flex-column gap-2">
              <label className="font-medium text-sm text-800">Tanggal</label>
          ...
          <div className="flex flex-column gap-2">
            <label className="font-medium text-sm text-800">URL Thumbnail</label>
