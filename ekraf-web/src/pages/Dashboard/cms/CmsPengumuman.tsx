...
const CmsPengumuman = () => {
  ...
  const breadcrumbItems = [{ label: 'CMS' }, { label: 'Pengumuman' }];
  ...
  const fetchData = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await pengumumanService.list({ page, per_page: 10, search: search || undefined });
      setItems(res.data);
  ...
  const confirmDelete = (id: number) => {
    confirmDialog({
      ...
      accept: async () => {
        try {
          await pengumumanService.destroy(id);
          fetchData();
        } catch {
  ...
  const statusBodyTemplate = (row: Pengumuman) => (
    <Tag value={row.is_published ? 'Published' : 'Draft'} severity={row.is_published ? 'success' : 'warning'} className="text-xs" />
  );

  const dateBodyTemplate = (row: Pengumuman) => (
    <span className="text-sm">{row.date ? new Date(row.date).toLocaleDateString('id-ID') : '-'}</span>
  );

  const actionBodyTemplate = (row: Pengumuman) => (
  ...
  const dialogFooter = (
    <div className="flex gap-2 pt-2 justify-content-end">
      ...
      <Button label={submitting ? 'Menyimpan...' : 'Simpan'} icon="pi pi-check" loading={submitting} onClick={() => document.getElementById('pengumuman-form')?.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }))} />
    </div>
  );
  ...
  return (
    <div className="flex gap-4 flex-column">
      ...
      <div className="flex flex-wrap gap-3 align-items-center justify-content-between">
        <div>
          <h2 className="m-0 text-2xl font-bold text-800">Pengumuman</h2>
          <BreadCrumb model={breadcrumbItems} home={breadcrumbHome} className="p-0 mt-2 border-none surface-ground" />
        </div>
        <Button label="Buat Pengumuman" icon="pi pi-plus" className="border-round-xl" onClick={openCreate} />
      </div>
      ...
      <Card className="shadow-1 border-round-xl">
        <div className="flex flex-wrap gap-3 mb-4 align-items-center justify-content-between">
          <span className="p-input-icon-left">
            ...
            <InputText value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Cari pengumuman..." className="border-round-xl w-20rem" />
          </span>
          <span className="text-sm text-500">Total: {total} pengumuman</span>
        </div>

        {error && <Message severity="error" text={error} className="w-full mb-3" />}

        <DataTable value={items} loading={loading} size="small" stripedRows responsiveLayout="scroll" emptyMessage="Belum ada pengumuman" className="border-round-lg">
      ...
      <Dialog visible={showModal} onHide={() => setShowModal(false)} header={editItem ? 'Edit Pengumuman' : 'Buat Pengumuman Baru'} footer={dialogFooter} style={{ width: '540px' }} modal className="border-round-xl" draggable={false}>
        <form id="pengumuman-form" onSubmit={handleSubmit} className="flex gap-3 pt-2 flex-column">
          <div className="flex gap-2 flex-column">
            <label className="text-sm font-medium text-800">Judul</label>
            <InputText value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} required className="border-round-lg" />
          </div>
          <div className="flex gap-2 flex-column">
            <label className="text-sm font-medium text-800">Konten</label>
            <InputTextarea value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} rows={5} required className="border-round-lg" autoResize />
          </div>
          <div className="grid">
            <div className="flex gap-2 col-6 flex-column">
              <label className="text-sm font-medium text-800">Tanggal</label>
              <InputText type="date" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} required className="border-round-lg" />
            </div>
            <div className="flex gap-2 col-6 flex-column">
              <label className="text-sm font-medium text-800">Status</label>
              <Dropdown value={form.is_published} options={[{ label: 'Published', value: true }, { label: 'Draft', value: false }]} onChange={e => setForm({ ...form, is_published: e.value })} className="border-round-lg" />
            </div>
          </div>
          <div className="flex gap-2 flex-column">
            <label className="text-sm font-medium text-800">URL Thumbnail</label>
            <InputText value={form.thumbnail} onChange={e => setForm({ ...form, thumbnail: e.target.value })} required={!editItem} placeholder="https://..." className="border-round-lg" />
          </div>
        </form>
      </Dialog>
    </div>
