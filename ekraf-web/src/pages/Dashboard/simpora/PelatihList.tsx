...
import { adminPelatihService } from '../../../services/newFeaturesService';
...
const PelatihList = () => {
  ...
  const [editItem, setEditItem] = useState<Partial<Pelatih> | null>(null);

  const breadcrumbHome = { icon: 'pi pi-home', command: () => navigate('/dashboard') };
  const breadcrumbItems = [{ label: 'Olahraga' }, { label: 'Database Pelatih' }];

  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const resp = await adminPelatihService.list({ search, status: statusFilter || undefined, page });
      setData(resp.data);
  ...
  const openEdit = (item: Pelatih) => { setEditItem({ ...item }); setShowDialog(true); };

  const saveItem = async () => {
    if (!editItem) return;
    try {
      if (editItem.id) {
        await adminPelatihService.update(editItem.id, editItem);
        toast.current?.show({ severity: 'success', summary: 'Berhasil', detail: 'Data pelatih berhasil diperbarui' });
      } else {
        await adminPelatihService.create(editItem);
        toast.current?.show({ severity: 'success', summary: 'Berhasil', detail: 'Data pelatih berhasil ditambahkan' });
      }
      setShowDialog(false);
      setEditItem(null);
      loadData();
  ...
  const deleteItem = async (id: number) => {
    try {
      await adminPelatihService.destroy(id);
      toast.current?.show({ severity: 'success', summary: 'Berhasil', detail: 'Data pelatih berhasil dihapus' });
      loadData();
    } catch {
  ...
  const namaTemplate = (row: Pelatih) => {
    const initials = row.nama.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase();
    return (
      <div className="flex align-items-center gap-3">
        <Avatar label={initials} shape="circle" style={{ background: 'linear-gradient(135deg, #14b8a6, #2dd4bf)', color: '#fff', fontWeight: 600, fontSize: '0.7rem' }} />
        <div>
  ...
  const statusTemplate = (row: Pelatih) => {
    const colors: Record<string, 'success' | 'warning'> = { aktif: 'success', nonaktif: 'warning' };
    return <Tag value={row.status} severity={colors[row.status] || 'info'} className="text-xs" />;
  };

  const actionTemplate = (row: Pelatih) => (
  ...
  return (
    <div className="flex flex-column gap-4">
      ...
      <div className="flex align-items-center justify-content-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-800 m-0">Database Pelatih</h2>
          <p className="text-sm text-500 mt-1 mb-0">Kelola data pelatih olahraga Kota Bontang</p>
        </div>
        <Button label="Tambah Pelatih" icon="pi pi-plus" className="border-round-xl" onClick={openNew} />
      </div>
      ...
      <Card className="shadow-1 border-round-xl">
        ...
        <DataTable value={data} loading={loading} paginator lazy rows={10} totalRecords={totalRecords} first={(page - 1) * 10} onPage={(e) => setPage(Math.floor((e.first ?? 0) / 10) + 1)} size="small" stripedRows responsiveLayout="scroll" emptyMessage="Belum ada data pelatih">
          <Column header="Nama" body={namaTemplate} />
          <Column field="lisensi" header="Lisensi" />
      ...
      <Dialog visible={showDialog} onHide={() => setShowDialog(false)} header={editItem?.id ? 'Edit Pelatih' : 'Tambah Pelatih'} style={{ width: '600px' }} className="border-round-xl">
        <div className="flex flex-column gap-3 mt-2">
          ...
          <div className="grid">
            ...
            <div className="col-6 flex flex-column gap-1">
              <label className="text-sm font-medium text-700">Lisensi</label>
              <InputText value={editItem?.lisensi || ''} onChange={(e) => setEditItem({ ...editItem, lisensi: e.target.value })} className="border-round-lg" />
            </div>
          </div>
          <div className="grid">
            <div className="col-6 flex flex-column gap-1">
